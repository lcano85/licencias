    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">{{ __('Assign Settlement') }}</h4>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        let table = $('#projectsData').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('get-assign-settlement.data') }}",
    ...
    function deleteUser(userId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/assign-settlement/delete/' + recID,
                    type: 'POST',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "recID": recID,
...
$(document).ready(function () {
    $("#projectDetails").on('submit', function (e) {
        ...
        $.ajax({
            url: "{{ route('assign-settlement.store') }}",
            method: "POST",
            data: formData,
            processData: false,
            contentType: false,
