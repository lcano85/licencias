    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header align-items-center d-flex">
                <h4 class="card-title mb-0 flex-grow-1">{{ __('Receipt Consecutive List') }}</h4>
                <div class="flex-shrink-0">
                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModalCenter"> + {{ __('Add Receipt Consecutive') }} </button>
                </div>
            </div>

            <div class="card-body">
                <table id="receiptConsecutiveDataTable" class="display table table-bordered table-responsive" style="width:100%">
                    <thead>
                        <tr>
                            <th>{{ __('Receipt Consecutive Name') }}</th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalCenterTitle">{{ __('Add Receipt Consecutive') }}</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row gy-4 mb-2">
                        <div class="col-xxl-12 col-md-12">
                            ...
                            <small class="text-muted">{{ __('This will be used as the next receipt number in the sequence.') }}</small>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#receiptConsecutiveDataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: { url: "{{ route('get-receipt-consecutive.data') }}" },
...
<script type="text/javascript">
    ...
    $(document).on('click', '.editBtn', function () {
        ...
        $.get("{{ url('/receipt-consecutive/edit') }}/" + editId, function (res) {
            if(res.success){
                const c = res.data;
                $('#consecutiveID').val(c.id);
                $('#consecutive_name').val(c.consecutive_name);
                $('#next_number').val(c.next_number);
                $('#exampleModalCenterTitle').text("{{ __('Edit Receipt Consecutive') }}");
    ...
    $('[data-bs-target="#exampleModalCenter"]').on('click', function () {
        ...
        $(".error-text").text('');
        $('#exampleModalCenterTitle').text("{{ __('Add Receipt Consecutive') }}");
    ...
    $(document).ready(function () {
        $("#consecutiveDetails").on('submit', function (e) {
            ...
            let url = (editId)
                ? "{{ url('/receipt-consecutive/update') }}/" + editId
                : "{{ route('receipt-consecutive.store') }}";
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire({ icon: 'success', title: '{{ __('Success') }}', text: response.message, showConfirmButton: true, timer: 2000 });
                        $("#consecutiveDetails")[0].reset();
                        $("#exampleModalCenter").modal('hide');
                        $('#receiptConsecutiveDataTable').DataTable().ajax.reload(null, false);
                        editId = null;
                    } else {
    ...
    function deleteStatus(consecutiveId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/receipt-consecutive/delete/' + recID,
                    type: 'POST',
                    data: { "_token": "{{ csrf_token() }}", "recID": recID },
                    success: function(response) {
                        Swal.fire('{{ __('Deleted!') }}', response.success, 'success')
                            .then(() => { $('#receiptConsecutiveDataTable').DataTable().ajax.reload(null, false); });
                    },
                    error: function(xhr) {
